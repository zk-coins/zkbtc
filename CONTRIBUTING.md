...
The specification establishes its own load-bearing invariants: the two-attack decomposition (Attack A / Attack B — private-fork mint settlement and Sybil-controlled operator epoch), the specific findings that close them (**R-04** for Attack A when a gatekeeper is designated; **R-09** growth-only cumulative operator set for Attack B), the effectively-trustless self-operator product claim (§1.1.1), the gatekeeper-independent-exit requirement, and the deployment taxonomy (**product default** = Corner D / self-operator + R-09; **gated** = Corner A; **closed enumeration** = Corner B; **UNSOUND** = Corner C = open registration + pooled **without** R-09 — the specification's §3.2.1.2 / §6.4). Do not silently restore the retired claim that a gatekeeper is required to close Attack B.

- Any change to the trust model, or to a load-bearing invariant (a finding that closes one of the two attacks, the gatekeeper-independent-exit requirement, or the four-profile deployment classification of a deployment profile — product default / gated / closed enumeration / unsound), **must** be justified **explicitly in the specification itself** and **must** go through **adversarial review** — a human reviewer with a security background actively trying to find a way the change reopens a closed attack or weakens a stated guarantee. These invariants must never be silently weakened, softened, or dropped.
- A change that removes or loosens a normative MUST / MUST NOT, narrows a stated residual, or reclassifies a deployment profile between any of the four classes (product default / gated / closed enumeration / unsound) is a **security-relevant** change. Treat it with the same scrutiny as a change to a cryptographic protocol — not as an editorial tweak.
