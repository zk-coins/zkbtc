...
3. **`reg_root_E` (epoch registration commitment).** `reg_root_E` = a Merkle commitment over S_E, the set of valid registration outputs admitted for epoch E under R-09 (each satisfying items 1–2 within E's registration window, **and** every **identity-party** from `reg_root_{E-1}` — same pubkey, or `pk_new` under a rotation signed by `pk_old`). Epoch signing keys of E−1 are **not** a superset obligation. It **MUST** be published on-chain as part of the epoch anchor (default: dedicated epoch-anchor `OP_RETURN` of §4.1.2.1; alternatively committed in the epoch's `MoveToBacked` graph-anchor leaf if that leaf commits the same 32 bytes) by the epoch's registering operators / coordinator. The in-circuit statement (via `C_lcp`) **MUST** establish: (i) `reg_root_E` is committed on a **PoW-valid header chain at adequate depth** (same LCP power as `MoveToBacked` — **canonicity is NOT established in-circuit**, exactly as §3.4); (ii) every included registrant satisfies items 1–2 (bond ≥ B, valid PoP, no duplicates); (iii) `agg_key_E` is the key-aggregation of exactly the **epoch signing keys** `{pk_{E,i}}` committed in `reg_root_E`; (iv) **R-09 superset:** every **identity-party** in `reg_root_{E-1}` is represented in `reg_root_E` (same pubkey or signed rotation; genesis epoch E=0 has no predecessor).
