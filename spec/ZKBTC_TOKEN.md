...
**Unique successor (MUST — R-09 item 1).** An `epoch_anchor_id` for epoch E is **invalid** if it confirms **before** that epoch's setup window ends (`W_reg` close + `W_setup` — §4.4). `J_E` is provisional until then; `reg_root_E` is published only at setup-window end (§4.1.2 item 6). A premature anchor (anyone hashing a candidate root during setup) **MUST NOT** win lineage. Among anchors that confirm **at or after** setup-window end, the unique successor of the current lineage head is the **first** valid `epoch_anchor_id` for this `asset_id` and next `E` on the chain used for the LCP, ordered by `(block height, transaction index, output index)` ascending. A later same-`E` anchor is a **sibling** and is **invalid**. In gatekeeper mode the gatekeeper's R-04 view **MUST** apply this first-valid-after-setup rule on its canonical chain. In no-gatekeeper mode the LCP proves the same property on the proven header chain (PoW-depth only — canonicity remains the Attack-A residual / `D_mint`). Mechanizing first-valid-after-setup in the converted graph is a **G3 acceptance criterion**.
