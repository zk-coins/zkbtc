...
1. **One set per asset.** There is a single S, not a per-epoch club that can restart. Epochs exist only as **presign batches** over the current S (and any joiners admitted in that window). **Uniqueness (MUST):** there is exactly one admitted `reg_root` lineage per `asset_id`. A `reg_root_E` is valid only as the unique successor of the unique predecessor already used by this asset (genesis `reg_root_0`, then each later E). Sibling `reg_root` values that do not spend/extend that unique head are **invalid**. Mint clause (e) **MUST** reject a stale or sibling `reg_root_E` that is not that unique current head (the successor of the unique `reg_root` used by the previous minted vault of this `asset_id`, or genesis). The byte encoding of that uniqueness is a launch pin (§4.1.2 item 7); the uniqueness rule is normative now.
