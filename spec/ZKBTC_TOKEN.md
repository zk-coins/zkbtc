...
6. **Anti-domination / anti-Sybil (MUST — R-09 is the Attack-B close; gate A(1) is weaker).** Policy P's bond + admissibility predicate **MUST** make single-party domination of S economically irrational when `|S| ≥ 2` (no single party ≥ half) **and MUST** encode R-09 (growth-only, no kick after admission, every new vault N-of-N over current S). When `|S| = 1` the half-fraction is **vacuous** (genesis self-operator — §4.1.2.1) and **MUST NOT** be treated as a violation. Otherwise a self-controlled **new** vault re-opens **Attack B**. The economic bond alone is **insufficient**. **R-09** is the close. In **gatekeeper mode**, Attack A (private-fork amortization) is blocked by **R-04**; optional R-08 is belt-and-suspenders, not the Attack-B close. **Corner C** (open registration + pooled **without** R-09) is **UNSOUND** (§3.2.1.2 / §5 residual 9 / §6.4).
...
Product-default field values (frozen into `asset_id` via `operator_set_root`): `join_rule = 0x01`, `r09_flags = 0x07`, `bond_class_sats = 200000000` (2 BTC, Clementine-class), `anti_dom_numer = 1`, `anti_dom_denom = 2`, `keyagg_rule = 0x01`, `pop_rule = 0x01`, `r08_duty = 0x00`. Changing any of these fields is a new policy and a new `asset_id`. The **per-epoch** bond amount `B_E` **MUST** be ≥ `bond_class_sats` and **MAY** be raised for a later epoch without changing `asset_id` (§4.4).

**Anti-domination (MUST — weaker than R-09).** When `|S| ≥ 2`, policy P **MUST** treat a `reg_root_E` as invalid if any single identity-party (one `identity_pubkey`, including after in-place rotation) is associated with ≥ `anti_dom_numer/anti_dom_denom` of the identity-parties in S_E. When `|S| = 1` this predicate is **vacuous** and **MUST NOT** be read as a domination violation: genesis self-operator (Corner D, the holder is S) is the product path. Anti-domination is an economic both-modes defence; **Attack B is closed by R-09**, not by this fraction.
...
leaf_payload :=
    ...
    bond_txid           ||  // 32; Bitcoin txid of the live bond output (internal byte order as in the transaction)
...
Leaves are ordered lexicographically by `identity_pubkey` (unique; duplicates **MUST** be rejected — item 2). The Merkle tree is the NEW-03 **left-complete balanced** binary tree over that ordered list: sibling hash `Hc("zkBTC/v2/RegNode", left || right)`; an odd last node is duplicated as the right child (same discipline as §3.1.1 tree shape). `reg_root_E` is the Merkle root.
...
**Epoch-anchor publication.** `reg_root_E` **MUST** appear on-chain as a 32-byte commitment in a dedicated epoch-anchor transaction's first `OP_RETURN` output (`0x6a` `0x20` ‖ `reg_root_E`), with the remaining outputs paying the epoch's unslashed-bond return connectors as the compiler template requires. Alternative commitment inside a `MoveToBacked` graph-anchor leaf is permitted only if that leaf commits the same 32 bytes; the dedicated `OP_RETURN` form is the **default pin**. Sibling/fork `reg_root` values that do not extend the unique published lineage are invalid (§3.1.2 item 1).

**Registration-output (bond) semantic pin.** The bond UTXO is a P2TR with the vault NUMS internal key of §3.1.1 (no key-path). Its script tree **MUST** contain exactly: (i) a **slash** path spendable only along the epoch's BitVM2 proven-fraud / grief-default graph (bond forfeited; sequencing-connector burn of §4.1 role-note (c)); (ii) a **return** path to the registrant's identity-keyed output after the epoch's claim/challenge horizon if unslashed. Exact compiler script bytes for (i) and (ii) remain item-7 launch-pinned. Amount **MUST** be ≥ `B_E`. No `pk_{E,i}` without a live bond output.
...
3. **Post the bond.** During the registration window of epoch E (§4.4), the user posts ≥ `B_E` sats (product default: 2 BTC class, `B_E = 200000000` sats unless a later epoch raised it) to a slashable registration output (§4.1.2 item 1 / §4.1.2.1). The bond is locked until that epoch's claim/challenge horizon if unslashed.
...
1. **Policy-P anti-domination and R-09 (open registration market).** Policy P's admissibility predicate **MUST** ensure that no single party controls ≥ half of S when `|S| ≥ 2` (§4.1.2.1; vacuous at `|S| = 1` — genesis self-operator is the product), **and MUST** encode R-09 (growth-only, no kick after admission, every new vault N-of-N over current S). Encoding: §4.1.2.1 (`r09_flags = 0x07`, `anti_dom_numer/denom = 1/2`, `bond_class_sats = 200000000`). Enforced economically by bond cost + honest-registration assumption; **Attack B is closed by R-09**, not by a gatekeeper vouch. (R-04 remains the private-fork / Attack A backstop when a gatekeeper is designated and is independent of this domination gate.) Bootstrap capture of **genesis** S (one organisation == entire S at first mint) reintroduces the prefix residual of R-09 item 6 — hence the first-mint rule.
...
10. **Open-operator anti-domination residual:** policy P **MUST** prevent single-party domination of S when `|S| ≥ 2` (§3.1.2 / §4.6A(1) / §4.1.1 / §4.1.2.1) **and MUST** encode R-09; otherwise a self-controlled **new** vault re-opens **Attack B**. When `|S| = 1` the half-fraction is vacuous (genesis self-operator). Economic bond/stake enforcement is weaker than R-09 and **insufficient alone**. **Prefix residual:** coins minted before the intended honest operator joined are untrusted because the token is fungible (§3.5.5); the first-mint rule of R-09 item 6 is load-bearing, not a footnote.
...
12. **Anti-domination bond calibration.** **Specified:** 2 BTC class + 1/2 identity fraction when `|S| ≥ 2`; vacuous at `|S| = 1` (self-operator genesis). **R-09** remains the Attack-B close. Live-economics confirmation is the remaining `PROVISIONAL` tag, not a missing rule.
...
| 2026-08-20 | Spec-side of G2 / REQ-4: canonical Policy-P and `reg_root_E` encoding (§4.1.2.1; 301-byte leaf including `pop_sig`), intended launch numbers for bond class (2 BTC), `B_E`, windows (`W_reg`/`W_round`/`W_setup`/`W_epoch`), holder→operator onboarding procedure (§4.1.3), and on-Bitcoin `slash_bounty` challenger funding (§4.1.4) with Clementine self-funded challenge named as the anti-pattern. Anti-domination vacuous at `|S| = 1`. A(2) resolved for the specified Groth16 path (MPC ceremony required). G3 acceptance criteria listed. **G2 and G3 boxes stay unchecked** — encodings and numbers are not an operational market and not a conversion demonstration. Residuals in §11 items 5–10 and 14 stay residuals, not new launch gates. |
