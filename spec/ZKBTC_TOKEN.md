...
**Bottom line (MUST state):** there is **no fully-clean no-gatekeeper profile for a pooled cross-graph reserve** — a **trust-minimized pooled cross-graph zkBTC REQUIRES a gatekeeper (or an equivalent external canonical+diversity oracle)** for **both** Attack A (R-04) and Attack B (R-08). No-gatekeeper + open = **UNSOUND**; no-gatekeeper + closed = **materially weaker** and not the open product. The product requirement (open operators + pooled fungible) **forces Corner A**. Cross-refs: §5 residual 9, §6.4, §6.6. **Scope note:** this trilemma concerns only the two **mint-settlement** drain attacks (A/B), which depend on the gatekeeper; the **full** reserve-safety conditions — 1-of-N setup honesty, an active honest challenger acting in-window, sound circuit/graph crypto, and the redeem-side payout-canonicity residual — are stated in the §5 trust matrix and summarised in the README's "When is zkBTC safe" statement, and the Corner-A "Sound" verdict is conditional on all of them, not only on the gatekeeper.
...
- **Payout binding and claim-marker uniqueness (MUST — NH-02 / NEW-02 / NEW-04):** the payout is an ordinary P2TR owned by `btc_recipient`; the operator **cannot** spend it (no key), so "consume the payout outpoint" is **not** a literal UTXO spend. Instead:
  ...
  4. **Non-transferable, claimant-committed, sole-funded payout (MUST — NEW-04).** Reimbursement fairness requires that the operator reimbursed from the vault is exactly the one that funded the payout to the holder, and that no other party can claim it. Attributing a Bitcoin payment's funding to one party proved subtle — three earlier formulations (gross/net input value-accounting, `SIGHASH_SINGLE` index-pairing, and a `SIGHASH_ALL`-only identity commitment) were each found insufficient in review — so this rule pairs a committed identity with a **mechanized sole-funder check**, and does **not** rely on any behavioural "an honest operator will not co-sign" assumption:
     - **Sole funder (MUST — the mechanized core).** **Every** input of the payout transaction **MUST** be authorised by the **claiming operator's registered identity** (its bonded key, or a funding-key set bound to it at registration). The claimant is therefore the transaction's **sole funder**; a payout transaction containing **any** input the claimant did not authorise is **not claimable**. This is a check on the transaction, not a behavioural assumption: a **co-signed** transaction in which a second party supplies most of the value while the claimant contributes only a dust input **fails** this rule, so a minority/dust claimant **cannot** ride on another operator's funding.
     ...
     **Why this closes reimbursement theft (mechanically, all four known variants).** (i) **Sole funder** makes the claimant the only funder of the transaction, so a co-signed multi-input payout in which another operator supplies the value is **not claimable** — this closes the co-signed zero-net-funding break **mechanically**, with no "honest won't co-sign" assumption. (ii) The **non-transferable identity commitment** means only the committed operator can claim, so a different operator cannot claim an honest operator's payout. (iii) **Non-malleability** (`SIGHASH_ALL` + SegWit-only inputs) prevents appending, relocating an already-signed input, duplicating the payout output, and `txid` mutation. Together these supersede the gross-input, net-output, index-pairing, and `SIGHASH_ALL`-only checks. **Honest status:** the exact encoding of the sole-funder input-authorisation check and of the identity commitment is a specific focus of the external audit (**G4**) and the Plonky2→BitVM2 conversion (**G3**), because reimbursement-funding attribution proved subtle across review rounds; the **invariant** — a non-transferable claim bound to the **sole funding** operator — is normative now.
...
- Any operator **MAY** fund the payout, but **MUST** be its **sole funder** — **every** input authorised by the operator's registered identity, no third-party inputs — committing the whole transaction with `SIGHASH_ALL` (§3.5.5 item 4 / NEW-04). A co-signed transaction in which another party supplies value is **not** claimable.
...
and a redeem transition proof such that:
 ...
 12. Non-transferable, claimant-committed, SOLE-FUNDED payout
     ...
     funder — it does NOT infer funding from input values. Rule:
       - SOLE FUNDER (mechanized core): every input of the payout
         transaction MUST be authorised by the claiming operator's
         registered identity (its bonded key / a funding-key set bound to
         it at registration); a payout tx with ANY input the claimant did
         not authorise is NOT claimable. This is a mechanized check, not
         a behavioural assumption: a co-signed tx where a second party
         supplies the value and the claimant adds only a dust input FAILS
         this rule;
     ...
     operator can claim) and SOLE-FUNDED (the claimant authorised every
     input, so a co-signed zero-net-funding ride is not claimable —
     closed mechanically, not by an "honest won't co-sign" assumption),
     and the tx is non-malleable (SIGHASH_ALL + SegWit-only inputs). This
     supersedes the gross, net, index-pairing, and SIGHASH_ALL-only
     checks. The exact sole-funder input-authorisation and identity-
     commitment encoding is a specific external-audit (G4) and conversion
     (G3) focus; the invariant (non-transferable claim bound to the sole
     funding operator) is normative now.
```
...
### 8.2 Peg-out observability (honest)
