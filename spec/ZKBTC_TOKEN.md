...
- **Inflate supply beyond the vault UTXO ceiling** — §3.6 conditional upper-bound auditability (**conditional on the reserve-safety assumptions of §5**: 1-of-N setup honesty + R-09 + R-04-when-gated + ≥1 honest challenger; a successful **Attack A**, **Attack B**, **or** unchallenged fraudulent/duplicate reimbursement breaks it).
...
| D2 | Mint-key derivation (`Pk_mint`; gatekeeper- vs depositor-base; MintKey domain; mod-n / even-y M-01) + mint consumes `Pk_mint` on **`MoveToBacked` vault** outpoint; gatekeeper verifies inputs **and own canonical Bitcoin view** then signs (not full proof); withholds until canonical `MoveToBacked` **and** `reg_root_E` (R-04 / Attack A) **and** until R-09 growth-only S holds (Attack B) | §3.2, §3.2.1.1, §4.1.2 |
