...
4. **Operators cannot steal** under 1-of-N setup honesty, ≥1 honest live challenger acting in-window, and sound circuit/graph crypto (REQ-4 / §5). Worst case under those assumptions is freeze, not theft. A critical soundness bug is the theft case. Sound circuit/graph crypto is a standing assumption (§5); there is **no** external-audit gate.
...
4. The holder completes Level-1 operator-set formation, then Level-2 per-deposit graph presign for every admitted deposit of the epoch, inside the setup window (§4.1.2 items 5–6). Already-admitted operators **MUST NOT** be dropped. A *joiner* that misses a per-round deadline is **excluded** from this join attempt and its registration bond is **forfeited/slashed**; that joiner was never in S. A new round then begins over `S_{E-1} ∪ (J_E minus the excluded joiner)`, with its own per-round deadline.
5. After the epoch's presigning is complete (Level-1 formation **and** Level-2 presign of all admitted deposits), the holder **MUST** delete the **epoch** signing secret and **MUST** keep the identity key.
6. Bonded-onboarding cost is real (§4.1 role-note (b)). Open registration is not a free click: the BitVM2 presign graph has cost. Do not assume every retail holder self-registers.
...
3. **Documented holder → operator onboarding** (self-fronting). The procedure in **§4.1.3** is the onboarding write-up: open permissionless registration (§4.1.1 / §4.1.2), first-mint rule, and the bonded-onboarding cost of §4.1 role-note (b). It does **not** instantiate the reference operator market (G2 remains operational/calibration).
4. **Challenger economics.** Naming the anti-pattern is **not** this gate clearing. See **Challenger economics** below. The *funding path* that makes in-window challenge rational remains open (`PROVISIONAL` / §11 item 4) until it exists.
...
Restatement of §4.0 as a checklist (two remaining gates — both **MUST** clear). There is **no** external-audit gate.
...
14. **Redeem-side payout-canonicity enforcement.** Whether the BitVM2 disprove path can enforce a **most-work (cumulative-work)** comparison so that a deep private-fork "payout" loses to an honest challenger's heavier canonical chain (§4.3.2 clause 8 / §5 residual 11). Open until the Plonky2→BitVM2 conversion (G3) demonstrates it; until then the redeem-side canonicity bound is **unmechanized** (no external anchor exists on the redeem side by design — REQ-4). There is **no** external-audit gate.
...
| 2026-08-20 | Added §4.1.3 as the holder → operator onboarding write-up and named the Clementine self-funded-challenge anti-pattern under §4.6A(4). Does **not** clear G2 operational instantiation, G3 conversion, or the REQ-4.6A(4) funding path (still §11 item 4). |
...
| D13 | Open operator registration market: permissionless join into cumulative S (R-09), bond/PoP/`reg_root_E`/MuSig2 KeyAgg/bounded restart rounds (exclusion only of not-yet-admitted joiners; per-round deadline ≠ setup window) + two-level ceremony (§4.1.2), holder→operator onboarding procedure (§4.1.3), be-your-own-exit-agent scoped to EXIT and Attack B, registration-free-reclaim residual → §10.2, first-mint rule §1.1.1 | §4.1.1, §4.1.2, §4.1.3, §1.1.1, §1.2, §4.3.1 |
...
| 'critical bug → only freeze/burn, not theft' stated unconditionally (§4.5) | Conflated a non-soundness bug's freeze/burn outcome with a circuit/graph soundness bug's theft risk; contradicted §5 'sound BitVM2 crypto' condition | Split by bug class: non-soundness bug = freeze/burn under 1-of-N + sound crypto; circuit/graph **soundness** bug can enable theft (§5 sound-crypto condition); soundness remains a standing assumption — there is **no** external-audit gate |

---

*End of design specification. No code. Not a production launch until the §4.6A REQ-4 gates and §4.6B maturity/integration gates clear (two remaining gates: Plonky2→BitVM2 conversion and open-registration market instantiation); the verifier itself is mainnet-proven. There is no external-audit gate.*
