...
2. **Join only (identity keys).** Anyone **MAY** add an **identity** key to S during a registration window (bond + PoP, §4.1.2). S **MUST NOT** shrink. The R-09 superset is over **identity keys**, not over per-epoch signing keys. A `reg_root_E` whose identity set omits any identity already in `reg_root_{E-1}` is **invalid** under P; mint clause (e) / two-level admission **MUST** reject it. Formation of `reg_root_E` **MUST** prove that every identity in `reg_root_{E-1}` is in `reg_root_E` (S_E = S_{E-1} ∪ J_E, J_E = new joiners; S_0 = J_0).
...
3. **`reg_root_E` (epoch registration commitment).** `reg_root_E` = a Merkle commitment over S_E, the set of valid registration outputs admitted for epoch E under R-09 (each satisfying items 1–2 within E's registration window, **and** every key in `reg_root_{E-1}`). It **MUST** be published on-chain as part of the epoch anchor (either committed in the epoch's `MoveToBacked` graph anchor leaf or a dedicated epoch-anchor transaction — launch-pinned form) by the epoch's registering operators / coordinator. The in-circuit statement (via `C_lcp`) **MUST** establish: (i) `reg_root_E` is committed on a **PoW-valid header chain at adequate depth** (same LCP power as `MoveToBacked` — **canonicity is NOT established in-circuit**, exactly as §3.4); (ii) every included registrant satisfies items 1–2 (bond ≥ B, valid PoP, no duplicates); (iii) `agg_key_E` is the key-aggregation of exactly the **epoch signing keys** `{pk_{E,i}}` committed in `reg_root_E`; (iv) **R-09 superset:** every **identity** pubkey in `reg_root_{E-1}` is in `reg_root_E` (genesis epoch E=0 has no predecessor).
