...
`operator_set_root` is a 32-byte commitment to this asset's **operator-registration policy** P (encoding of the policy tree is a launch parameter; the root value is frozen in `asset_id`). **Default and product** policy P is **open, permissionless, and growth-only (R-09)**: anyone **MAY** join S by posting the policy's bond and a pubkey with proof-of-possession (§4.1.2); there is **no allowlist and no admin approval**; **nobody leaves**. Under that default it is a **policy** root, **not** a frozen enumeration of operator keys — the concrete set S grows over time and **MUST** be the operator set of every new vault (§4.1.1). **Alternative (non-default):** a *closed genesis-enumerated* policy P′ (fixed key list + gate-A(1) diversity at launch) is a **distinct policy P** (distinct `operator_set_root` / `asset_id`) usable only in the Corner-B profile of the trilemma (§3.2.1.2 / §6.4) — it closes Attack B by enumeration but **abandons open registration** and does **not** cleanly close Attack A. Concrete registration-window length, epoch cadence, exact bond amount within the bond class, and setup-window deadline are **per-epoch bridge parameters** (§4.4) and do **not** change `asset_id`.

**R-09 — cumulative growth-only operator set (MUST — Attack B close).** Let S be the admitted operator set of this `asset_id`.

1. **One set per asset.** There is a single S, not a per-epoch club that can restart. Epochs exist only as **presign batches** over the current S (and any joiners admitted in that window).
2. **Join only.** Anyone **MAY** add a key to S during a registration window (bond + PoP, §4.1.2). S **MUST NOT** shrink. A `reg_root_E` that omits any pubkey already in `reg_root_{E-1}` is **invalid** under P; mint clause (e) / two-level admission **MUST** reject it. Formation of `reg_root_E` **MUST** prove that every operator in `reg_root_{E-1}` is in `reg_root_E` (S_E = S_{E-1} ∪ J_E, J_E = new joiners; S_0 = J_0).
3. **No leave, no kick after admission.** An already-admitted operator **MUST NOT** be excluded from S, including for Level-1/Level-2 griefing of a later epoch. Grief-default **MAY** slash the bond; it **MUST NOT** drop the key. Exclusion **MAY** apply only to a joiner in J_E who has **not yet** been admitted to S (failed the current join round). Kick-after-admission re-opens Attack B (honest keys griefed out, then a Sybil-only vault).
4. **Key rotation in place (MAY).** An admitted operator **MAY** replace `pk_old` with `pk_new` iff `pk_old ∈ S` and `pk_old` signs the rotation. Cardinality of S is unchanged; the party remains. Rotation **without** the old key is **leave** and is **forbidden**. Lost keys are dead weight: new vaults cannot complete N-of-N until that party rotates or the mint of new vaults stays frozen. **Safety of existing coins over mint liveness.**
5. **Every new vault is N-of-N over current S.** `agg_key_E` **MUST** be the MuSig2 KeyAgg of exactly S_E. An honest member of S **MUST NOT** sign a malicious graph. Once ≥1 honest operator is in S, no later vault can have a malicious graph.
...
**Signing order / mint-signing checklist (normative — R-03 / R-04 / R-08; MUST).** "Verify the full mint proof before signing" is **circular**: the completed proof `C` embeds the gatekeeper's BIP-340 signature over `m_state` (S2C over `H(ProofData)`), so the proof cannot exist before the signature. The correct order is:
...
Aggregate circulating supply therefore remains a **conditional upper bound** (`circulating ≤ current backing vault balance` under the reserve-safety assumptions of §5: **1-of-N + R-09 + R-04-when-gated + ≥1 honest challenger**; a successful Attack A, Attack B, **or** unchallenged fraudulent reimbursement breaks it). It is **not** exactly computable from chain data alone.
...
1. **Open join into cumulative S (permissionless, MUST — R-09).** There is one operator set S per asset. For each deposit epoch E there is a **registration window** before that epoch's vault graphs are set up. **Anyone** **MAY** join S by (i) posting the policy-P bond to a bonded/slashable output and (ii) contributing their operator pubkey to the epoch's N-of-N presigning ceremony over **current S ∪ {joiners}**. No allowlist, no admin approval, no gatekeeper approval (the gatekeeper has **NO** role here — that is mint-only). A holder **MAY** register to be their **own** exit agent and, if they join **before the first mint**, their own Attack-B honest signer (§1.1.1). **S never shrinks.** A new epoch that omitted any already-admitted operator is **invalid**.

2. **Epoch setup ceremony (Level 1 — §4.1.2).** S_E = S_{E-1} ∪ J_E runs the BitVM2 N-of-N formation ceremony (bond/PoP/`reg_root_E`/KeyAgg; bounded restart rounds on **not-yet-admitted** joiners only), then Level-2 per-deposit graph presign for each deposit, then all delete signing shares (NEW-01). `agg_key_E` and `reg_root_E` are formed per **§4.1.2** (MuSig2 KeyAgg over S_E; Merkle commitment of valid bonded registrations with PoP **and** the superset proof against `reg_root_{E-1}`).

3. **Freeze semantics (MUST state honestly).** After the epoch freeze, S_E is fixed **for reclaim from E's vault** (the pre-signed graph binds those keys — this is the covenant-emulation reality; registration-free reclaim needs §10.2 covenants). Registration for **future** epochs stays permanently open as **join-only**: new operators add to S, old ones stay. Any given epoch's vault is reclaimable by that epoch's S_E (a prefix of later S). Fungibility of the **claim** (`asset_id`) is unaffected: the token is one fungible asset; redeem is cross-graph across all in-set epoch vaults (§4.3) — which is why the first-mint rule of R-09 item 6 is load-bearing.

4. **"Be your own exit agent" — EXIT and Attack B (MUST state).** Because registration is open, a holder who wants a guaranteed self-serviced **exit** **MAY** register (once, into S) and thereafter front + reclaim their own redemptions. Because S is growth-only, that same registration is the Attack-B close: every later vault includes them. **Scope (MUST):** "no party can prevent this" refers to a **registered holder serving their own EXIT** along a graph **already presigned at that deposit's setup**, **and** to that holder refusing a malicious graph at setup of any later vault. Exit reclaim does not run a live ceremony at exit time, so exit cannot be griefed once the graph exists. Setup-time (peg-in / registration) griefing is handled by §4.1.2 item 5 (Level-1 restart rounds **for not-yet-admitted joiners only**) and the deposit-taproot refund leaf (Level-2) — **not** by this exit claim. This is the concrete meaning of REQ-4's open-operator property **and** of §1.1.1's effectively-trustless claim.
...
3. **`reg_root_E` (epoch registration commitment).** `reg_root_E` = a Merkle commitment over S_E, the set of valid registration outputs admitted for epoch E under R-09 (each satisfying items 1–2 within E's registration window, **and** every key in `reg_root_{E-1}`). It **MUST** be published on-chain as part of the epoch anchor (either committed in the epoch's `MoveToBacked` graph anchor leaf or a dedicated epoch-anchor transaction — launch-pinned form) by the epoch's registering operators / coordinator. The in-circuit statement (via `C_lcp`) **MUST** establish: (i) `reg_root_E` is committed on a **PoW-valid header chain at adequate depth** (same LCP power as `MoveToBacked` — **canonicity is NOT established in-circuit**, exactly as §3.4); (ii) every included registrant satisfies items 1–2 (bond ≥ B, valid PoP, no duplicates); (iii) `agg_key_E` is the key-aggregation of exactly S_E committed in `reg_root_E`; (iv) **R-09 superset:** every pubkey in `reg_root_{E-1}` is in `reg_root_E` (genesis epoch E=0 has no predecessor).
...
6. **Two-level ceremony architecture (MUST).** Resolve the epoch-ceremony vs per-deposit-ceremony split explicitly:
   ...
   - **Level 1 — epoch operator-set formation (once per epoch):** the registration + anti-grief ceremony of item 5 fixes S_E → `reg_root_E`, `agg_key_E`. A Level-1 griefer who is a **not-yet-admitted joiner** is excluded + slashed + restart (item 5). A Level-1 griefer who is **already in S** is slashed if the policy so provides, **not** dropped; the epoch does not form without them.
   - **Level 2 — per-deposit graph presign (once per deposit, by S_E):** each deposit's BitVM2 transaction graph (§4.2 step 3) is presigned by the epoch's N-of-N set.
     ...
     - **Systematic Level-2 griefing by a not-yet-admitted joiner → Level-1 exclusion of that joiner (MUST).** Systematic Level-2 griefing by an **already-admitted** operator **MUST NOT** drop them from S (R-09). Their bond **MAY** be slashed; new vaults stall until they presign or rotate. Dropping them would re-open Attack B.
...
**The gatekeeper is optional. It is not what makes zkBTC effectively trustless.** Attack B and operator-theft safety are closed by R-09 and 1-of-N (§1.1.1 / §3.1.2). Do **not** describe the gatekeeper as a required trusted party for pooled backing.
...
**Ship condition.** A zkBTC **MUST NOT** be shipped as open registration + pooled **without** R-09 (**Corner C** is **UNSOUND**). The product ships as Corner D (R-09, optional gatekeeper). The first-mint rule of R-09 item 6 **MUST** be implemented in wallets.
...
|----------|--------------------------|----------------------|
| Supply audit | Operator-attested; amounts unobservable | Conditional upper bound vs public vault UTXOs (**conditional on the reserve-safety assumptions of §5**: 1-of-N + R-09 + R-04-when-gated + ≥1 honest challenger — broken by Attack A, Attack B, **or** unchallenged fraudulent reimbursement); exact via optional aggregate attestation (§3.6) |
