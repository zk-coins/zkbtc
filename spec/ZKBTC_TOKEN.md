...
| `D_mint` | ~2016 blocks (`PROVISIONAL`) | Bridge security horizon; long finality OK per REQ-4 | Mint LCP: `MoveToBacked` confirmation depth lower bound (§3.3(e)); private-fork mining for mint value economically irrational at this depth. **Circuit-pinned (v2 release)** — changing it is a new-digest circuit-parameter release (§3.7.2 / §11 item 6), not a per-epoch value |
