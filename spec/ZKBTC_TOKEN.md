...
| **REQ-4** | **Gatekeeper-independent, open-operator, liveness-bounded redemption:** every holder **MUST** be able to redeem for on-chain BTC without the gatekeeper's permission or technical cooperation. The exit (peg-out) agent role is **open and permissionless** — anyone **MAY** register (bond + pubkey, §4.1.1) and serve exits, including a holder acting as **their own** exit agent. Exit depends only on the liveness of **≥1 registered operator** (BitVM-family residual — §1.2, §4.3), never on a fixed or privileged operator, and no operator can steal (1-of-N setup honesty + ≥1 honest live challenger acting in-window — worst case freeze/burn (under sound circuit/BitVM2-graph crypto — a critical soundness bug can instead enable theft, §4.5); §5). High minimum denominations and multi-week wait windows are acceptable for emergency exit. |
...
**Open-operator resolution.** On today's Bitcoin the exit-agent role can be made fully **open and permissionless** — anyone **MAY** post a bond and register as an operator for a deposit epoch (§4.1.1), so a holder can be **their own** exit agent and no operator set is a closed club. What is NOT achievable without a covenant soft-fork is *registration-free* unilateral self-reclaim: a holder who never registers relies on ≥1 *registered* operator being online to front and reclaim (a liveness dependency, not custody — that operator cannot steal under 1-of-N + ≥1 honest live challenger acting in-window (under sound circuit/BitVM2-graph crypto — a critical soundness bug can instead enable theft, §4.5) (§5)). This design delivers the maximal open-operator property available on today's Bitcoin and states the registration-free residual plainly (§10) rather than masking it.
...
2. **zkBTC bridge profile** — off-circuit **BitVM2**-based (BitVM-family) construction that holds the BTC reserve. Peg-in is permissionless for minters, optionally gated by a per-asset gatekeeper that is entry quality filter, **canonical-chain anchor** (R-04 / Attack A), and **operator-set-diversity vouch** (R-08 / Attack B) for mint settlement (REQ-3). Peg-out is gatekeeper-independent (REQ-4), operator-fronted (liveness only — operators **cannot steal** under 1-of-N + ≥1 honest live challenger acting in-window (§5) (under sound circuit/BitVM2-graph crypto — a critical soundness bug can instead enable theft, §4.5)), with **optimistic assert/challenge/disprove** reimbursement and **permissionless challenging** (BitVM2 core property; availability ≠ guaranteed action — §4.1 role-note (a)).
...
**Key-deletion semantics and the pre-deletion window (MUST — NEW-01 clarification).** "Delete signing shares" means each honest operator **MUST** irretrievably destroy the secret key material that could produce **any new signature** under `agg_key_E` — the long-term signing capability, not merely a MuSig2 round nonce or partial signature. Because a single `agg_key_E` is reused to Level-2 presign **every** deposit graph of epoch E (§4.1.2), an honest operator retains its share until E's presigning is complete (the last admitted deposit's graph is signed) and then deletes; a deposit arriving after an honest operator has deleted for E cannot be admitted to E and is routed to a later epoch. **During the pre-deletion window** (E's registration/presign phase, before deletion), the full N-of-N coalition could still sign — so safety **in that window** rests on the stronger no-full-collusion assumption, not on 1-of-N. **After** E's presigning completes and **≥1 honest operator has deleted**, 1-of-N setup honesty holds: no coalition can authorise any spend outside the already-pre-signed graph. This window is the covenant-emulation reality (matching the per-epoch batch model of BitVM2/Clementine); it is an accepted, stated assumption, not a silent gap.
...
**No live CHECKSIG on the vault (MUST — NEW-01; covenant-emulation core).** Every vault spend path **MUST** move funds **only along the pre-signed BitVM2 transaction graph** fixed at setup. The vault **presigning set MUST be N-of-N** (all sign; all delete — §2.1). After the **epoch's** presigning is complete (all admitted deposits' graphs signed — §2.1 epoch-wide pre-deletion window), each signer's secret key material able to produce **any new signature** under the vault aggregate key (the long-term signing capability, not merely a MuSig2 nonce) for those graph transactions **MUST** be **deleted** (NEW-01; deletion semantics and the epoch-wide pre-deletion window — §2.1). Under 1-of-N setup honesty, one honest deletion means no coalition can authorise a spend outside the approved graph.
...
**Two independent *mint-settlement* backing-drain attacks (normative).** These are the two backing-drain attacks that arise at **mint settlement** and that a gatekeeper closes. They are **not** the only reserve-safety conditions: the **redeem/reimbursement** side additionally requires the §4.3.2 fraud statement + ≥1 honest active challenger (else unchallenged-fraud drain — §3.6) and carries the payout-canonicity residual (§5 residual 11). "Two" scopes the **mint-side** decomposition, not an exhaustive list of every reserve-safety condition.
...
|--------|------------|---------|-------|
| **A (RECOMMENDED — this is zkBTC)** | Open registration + pooled + a gatekeeper (gives up no-gatekeeper) | **Sound — conditional on an honest, correct gatekeeper performing R-04/R-08** (a trust assumption, not a cryptographic closure — R-08 is off-protocol identity/reputation vetting; a compromised or honest-but-fooled gatekeeper re-opens the attack — §6.3) | Both attacks cleanly closed: R-04 closes Attack A (covers `MoveToBacked` **and** `reg_root_E` — §4.1.2); R-08 closes Attack B. Primary product profile. |
...
The LCP proves `MoveToBacked` at depth ≥ `D_mint` on a PoW-valid header chain (deep enough that private-fork mining for the mint value is prohibitively costly and very unlikely to succeed for a sub-majority miner as a pure mining attack) **and** the N-of-N witness (clause (e) point 3) **and** operator-set membership (clause (e) point 4), including that `reg_root_E` is committed on a PoW-valid chain at adequate depth. That is **necessary but not sufficient** to bind the mint to the **canonical** Bitcoin chain: the LCP proves confirmation on *some* header chain, not that that chain is the verifier's canonical tip — for **either** `MoveToBacked` **or** `reg_root_E`.
...
- Depth ≥ `D_mint` makes pure private-fork fabrication of `MoveToBacked` prohibitively costly and very unlikely to succeed at the mint value for a sub-majority miner; the N-of-N witness excludes refund-leaf private-fork forgeries without full operator collusion at setup. **Neither alone binds the proven chain to the canonical tip** — that is the gatekeeper's mint-signing duty (R-04), covering **both** `MoveToBacked` **and** `reg_root_E`.
...
- **Payout binding and claim-marker uniqueness (MUST — NH-02 / NEW-02):** the payout is an ordinary P2TR owned by `btc_recipient`; the operator **cannot** spend it (no key), so "consume the payout outpoint" is **not** a literal UTXO spend. Instead:
  ...
  4. **Non-transferable, claimant-committed payout (MUST — NEW-02).** Reimbursement fairness requires that the operator reimbursed from the vault is exactly the one that funded the payout to the holder, and that no other party can claim it. Attributing a Bitcoin payment's funding to one party proved subtle — two earlier formulations (gross/net input value-accounting, and `SIGHASH_SINGLE` index-pairing) were found insufficient in review — so this rule binds the claim to a **committed identity** rather than inferring funding after the fact:
     - the payout transaction **MUST** commit, in a **single, hiding, binding claimant-identity commitment** (e.g. `H(claiming operator's bonded key ‖ redeem ID (Pkᵢ, Rᵢ) ‖ blind)` in an `OP_RETURN` output or an equivalent field, **opened only in the reimbursement claim** — exact encoding launch-pinned, NEW-03 honesty). Because the commitment is **hiding**, the payout does **not** reveal the redeem↔payout link on-chain until the claim opens it (preserving the §8.2 indistinguishability-until-claim boundary); a payout transaction carrying **zero or more than one** claimant commitment is **not** claimable;
     ...
     - the committed claimant's bonded key **MUST** sign the payout transaction under a **full-transaction commitment** (`SIGHASH_ALL` — all inputs and all outputs; **not** `ANYONECANPAY`);
     ...
     - the reimbursement fraud statement **MUST** verify the **claiming** operator's bonded key **equals** the key committed in the payout transaction; a claim by any other key **fails**.
     **Why this closes reimbursement theft (all three known variants).** The claim is **non-transferable** — only the committed operator can claim, so a different operator cannot claim an honest operator's payout (closes the co-signed multi-claimant break). The transaction is **non-malleable** (`SIGHASH_ALL` + SegWit-only inputs), so no third party can append, relocate an already-signed input, or duplicate the payout output, and the `txid` cannot be mutated (closes the BIP-341 pair-relocation break and the residual `txid` malleability). And an honest operator will **not** co-sign a payout that commits a *different* claimant, so a zero-net-funding attacker cannot induce an honest operator to fund a payout it then claims — the committed claimant must fund the payout itself. This supersedes the gross-input, net-output, and index-pairing checks. **Honest status:** the exact encoding of the claimant-identity commitment and the input-provenance check is a specific focus of the external audit (**G4**) and the Plonky2→BitVM2 conversion (**G3**), because reimbursement-funding attribution proved subtle across review rounds; the **invariant** — a non-transferable claim bound to the operator that funds the payout — is normative now.
...
- Any operator **MAY** fund the input side from **its own funds**, committing the whole transaction with `SIGHASH_ALL` (each operator self-funds a complete, non-malleable payout — no third-party input recombination).
...
5. **Irreducible residual (MUST state plainly).** A holder who **never** registers relies on ≥1 registered operator of some in-set epoch being online to front + reclaim. That is a **liveness** dependency, not custody — the operator cannot steal (1-of-N + ≥1 honest live challenger acting in-window; worst case freeze/burn (under sound circuit/BitVM2-graph crypto — a critical soundness bug can instead enable theft, §4.5); §5 residual 3). Registration-free unilateral self-reclaim from a pooled vault is **covenant-class** and not available on today's Bitcoin (§10.2).
...
3. **Presign (MUST — NEW-01).** The epoch's **openly-registered operators** (§4.1.1) generate and **N-of-N** **pre-sign the entire per-deposit BitVM2 transaction graph** (`MoveToBacked`, assert/challenge/disprove/reimbursement/payout connectors — **no** cooperative-refund transaction on the backing-only vault). Content is fixed at setup → txids fixed → the downstream reimbursement graph stays pre-signable. After the **epoch's** presigning is complete (all admitted deposits — §2.1), each signer's long-term signing capability under the epoch aggregate key **MUST** be **deleted** (NEW-01 deletion semantics + pre-deletion window — §2.1), not merely its MuSig2 nonces. The presigning set is **N-of-N**, not a `t < N` threshold (NEW-01 — §2.1).
...
1. **Deposit vault path** spends only into the **pre-signed** `MoveToBacked` under the N-of-N aggregate / MuSig2 path as required by the reference construction — **not** via the user's refund leaf. The aggregate key **MUST** be admitted by `operator_set_root` (policy → epoch `reg_root` → `agg_key`). Signatures for `MoveToBacked` are produced at setup; each signer's long-term signing capability under the epoch aggregate key **MUST** be deleted after the **epoch's** presigning completes (NEW-01 deletion semantics + epoch-wide pre-deletion window — §2.1), not merely its MuSig2 nonces. Firing **MUST** require the co-sign gate of §4.2 step 1: **depositor co-signature in both modes** (INV-01); **gatekeeper co-signature additionally** in gatekeeper mode.
...
| Payout outpoint reused across claims | Counterprovable by first-marker of earlier valid claim-marker tuple (NH-02; logical marker, not UTXO spend); the payout tx commits the claimant's identity + redeem ID and is non-malleable (SegWit-only + `SIGHASH_ALL`), so the claim is non-transferable to another operator (NEW-02) | Safety (1-honest watchtower) |
| Claim hijack (free-rider, relocation, duplicate-payout, or co-signed multi-claimant) on another's payout | Fails the **non-transferable claimant-committed payout** rule: the payout tx commits the funding operator's bonded key + redeem ID and is non-malleable (SegWit-only + `SIGHASH_ALL`); only the committed operator can claim it, and an honest operator will not co-sign a payout naming a different claimant (NEW-02) | Safety |
...
and a redeem transition proof such that:
 ...
 12. Non-transferable, claimant-committed payout (MUST — NEW-02): the
     fraud statement MUST prove the claim is bound to a committed
     claimant identity, NOT infer funding from input values. Rule:
       - the payout transaction commits, in a SINGLE, HIDING claimant-
         identity commitment (a blinded commitment to (claiming
         operator's bonded key, redeem ID (Pkᵢ, Rᵢ)) in an OP_RETURN or
         equivalent field, opened only in this claim; encoding
         launch-pinned; a tx with zero or multiple such commitments is
         not claimable);
       - ALL payout-transaction inputs are SegWit (v0/Taproot; empty
         scriptSig), so txid is non-malleable;
       - the committed claimant's bonded key signed the payout tx under
         a full-transaction commitment (SIGHASH_ALL; all inputs and
         outputs; NOT ANYONECANPAY);
       ...
       - the CLAIMING operator's bonded key EQUALS the key committed in
         the payout tx (a claim by any other key fails).
     Why sound: the claim is NON-TRANSFERABLE (only the committed
     operator can claim), the tx is non-malleable (SIGHASH_ALL +
     SegWit-only inputs, so no append/relocate/duplicate and no txid
     mutation), and an honest operator will not co-sign a payout that
     commits a different claimant, so a zero-net-funding attacker cannot
     ride on an honest operator's payment. This supersedes the gross,
     net, and index-pairing checks. The exact commitment/provenance
     encoding is a specific external-audit (G4) and conversion (G3)
     focus, because reimbursement-funding attribution proved subtle; the
     invariant (non-transferable claim bound to the funding operator) is
     normative now.
```
...
| D5 | Redeem: `redeem_commitment` incl. `max_fee < redeem_amount`, redeem ID `:= (Pkᵢ, Rᵢ)`, claim-marker uniqueness (logical first-marker on valid unchallenged set; slashed excluded; non-transferable claimant-committed payout (identity commitment + SegWit-only + `SIGHASH_ALL`)), value preservation (exactly `redeem_amount` from vault), payout ≥ `redeem_amount − max_fee` | §3.5, §4.3.2 |
...
| no-gatekeeper same-epoch reimbursement escape | Unconstructible (`redeem_commitment` has no epoch field; vault-outpoint binding ruled out §3.5.5) and insufficient (multi-vault Sybil epoch still cross-graph vulnerable) | Sound-deployment **trilemma** (Corner A recommended (sound conditional on an honest, correct gatekeeper performing R-04/R-08 — §3.2.1.2 / §6.3); Corner B materially weaker Attack-A operator-oracle-only; Corner C forbidden/UNSOUND; no fully-clean no-gatekeeper pooled profile) |
