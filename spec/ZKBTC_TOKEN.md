...
**Status:** Design specification. No code. **Buildable on today's Bitcoin — no soft-fork.** The normative fraud-proof verifier is **BitVM2** (mainnet-proven; reference exemplar: Citrea Clementine, live since 2026-01; Bitlayer BitVM bridge mainnet since 2025-07). Remaining work is **two gates**: (a) the **Plonky2 → BitVM2 (SNARK/Groth16) conversion of the zkCoins compliance predicate demonstrated** (§4.0 / §4.6B / G3), and (b) the **open operator-registration market operational** (policy encoding, bond / anti-domination, `reg_root_E` format, ceremony — semantic requirements in §4.1.2 / §4.1.2.1; launch numbers §4.4; onboarding §4.1.3; challenger funding §4.1.4). Spec-side encodings and numbers do **not** by themselves clear G2 or G3. Both gates, plus §4.6A REQ-4 gates, **MUST** clear. zkBTC is a substantial engineering project (Citrea-class, order of months) built on **proven** components — not speculative research and not contingent on any not-yet-mainnet construction. Glock / BitVM3 / Mosaic are retained only as **§10 future efficiency upgrades** (BitVM3/Mosaic/Argo preserve permissionless challenge; Glock carries a designated-verifier trade-off — §10.1).
...
4. **Operators cannot steal** under 1-of-N setup honesty, ≥1 honest live challenger acting in-window, and sound circuit/graph crypto (REQ-4 / §5). Worst case under those assumptions is freeze, not theft. A critical soundness bug is the theft case. Sound circuit/graph crypto is a standing assumption (§5).
...
| **Gatekeeper** (optional) | Designated blockchain address — an x-only BIP-340 key; **MAY** be a MuSig2/FROST **threshold** aggregate of a holder committee (still a single x-only key in-circuit). Per-mint approval is mandatory when designated. Performs **three load-bearing roles** when designated (§6.1, §3.2.1.1): (a) **source-of-funds screening**, (b) **canonical-chain anchor (R-04)** — withholds the `Pk_mint` mint-settlement signature until **both** `MoveToBacked` **and** the epoch's `reg_root_E` are confirmed on the gatekeeper's **own canonical Bitcoin view** (closes R-04 / Attack A for both anchors), (c) **epoch-relative R-09 withhold** — **MUST** withhold on a sibling/fork `reg_root`; **MUST NOT** withhold an ancestor-epoch mint solely because a later epoch head is published. This withhold is **additional**; **Attack B is closed by in-circuit R-09**, not by the gatekeeper. It **MAY** additionally issue an **operator-set-diversity vouch (R-08)** as belt-and-suspenders. | Entry filter + mint-settlement canonicity gate; **cannot mint without valid backing** (deposit + operator-set-legit clauses) and **cannot redirect another depositor's committed mint** (clause (g)); has no special minting privilege beyond any depositor; **cannot** freeze, claw back, or touch existing / circulating coins, and **cannot** block exit (REQ-4 intact). **When the gatekeeper performs R-04 honestly:** it cannot inflate; **cannot settle a mint against a non-canonical `MoveToBacked` or substituted `reg_root_E`**. A **compromised / negligent** gatekeeper that **skips** R-04 **can enable Attack A**; skipping optional R-08 does **not** reopen Attack B if R-09 holds. Gatekeeper **integrity** is a backing-safety dependency **for Attack A (and SoF) when designated** (§6.3); it **cannot** directly forge a vault spend, steal circulating coins, freeze, or block exit. The gatekeeper is **not** the reason zkBTC is effectively trustless. |
...
4. **Key rotation in place (MAY).** An admitted operator **MAY** replace identity `pk_old` with `pk_new` iff `pk_old ∈ S` and `pk_old` signs the rotation. Cardinality of S is unchanged; the party remains. Rotation **without** the old identity key is **leave** and is **forbidden**. Lost **identity** keys are dead weight: rotation requires `pk_old`, so a lost identity key **permanently** freezes **new** vaults. Already-presigned vaults remain 1-of-N-exitable. **Safety of existing coins over mint liveness.** Epoch signing keys are a different object: they are freshly generated per epoch and **deleted** after that epoch's presign (NEW-01) — that deletion **MUST NOT** be treated as leave.
...
1. Policy P **MUST** encode R-09 (growth-only S, superset `reg_root_E`, no kick after admission, every new vault N-of-N over current S) **and** the launch-gate-A(1)-class economic anti-Sybil / anti-domination constraint (§4.6A / §4.1.2.1): no single **economic** party should control ≥ half of S, enforced by bond cost (`ceil(|S|/2) · B_E`) + the honest-registration assumption — **not** by an in-circuit unique-key headcount (that headcount would reject every `|S| = 2` set). When `|S| = 1` the fraction is **vacuous** and **MUST NOT** be read as a domination violation (genesis self-operator is the product path). Anti-domination is a **weaker, both-modes** defense and is **insufficient alone** against Attack B under pooled cross-graph reimbursement; R-09 is the close.
...
**Honest cost of growth-only S:** a **lost** identity key **permanently** freezes **new** vaults (rotation requires `pk_old`). A **grieving** admitted operator who still holds `pk_old` freezes **new** vaults until they rotate a live key or resume presign. Already-presigned ancestor-epoch vaults remain mintable (R-09 item 1). **Existing** coins remain exit-able along that vault's already-presigned 1-of-N graph. That is the safety-over-mint-liveness trade-off and **MUST** be stated, not hidden. Attack A remains closed **cleanly only** by R-04 (gated mode) — independent of R-09.
...
2. **Vault/operator-set legitimacy (MUST — R-09; optional R-08 belt-and-suspenders):** the vault instance is legitimately constituted under this asset's `operator_set_root` policy as a **growth-only successor of its predecessor epoch** (R-09 — §3.1.2 items 1–2). The R-09 check is **epoch-relative** to this mint's epoch: `reg_root_E` is on this asset's unique published epoch lineage and omits nobody already in `reg_root_{E-1}`, and `agg_key` is KeyAgg of the **epoch signing keys** `{pk_{E,i}}` of **S_E** (NEW-01). The gatekeeper **MUST** reject a sibling/fork `reg_root`. It **MUST NOT** withhold solely because a later epoch head is already published or because later joiners are missing from an ancestor-epoch `reg_root_E`. **Attack B is closed by R-09**, not by the gatekeeper. The gatekeeper **MAY** additionally refuse to sign unless it has independently vouched that S_E has **at least one independent honest signer (1-of-N-honest basis)** (optional **R-08** identity/reputation check). R-08 is mint/entry-time only; it does **not** gatekeep registration or exit (REQ-4 intact) and **MUST NOT** be described as the Attack-B close;
...
4. **Canonical-chain anchor (MUST — closes R-04 / Attack A; covers `MoveToBacked` and `reg_root_E`).** On the gatekeeper's **own canonical Bitcoin view**: (i) the relevant `MoveToBacked` is confirmed to depth ≥ `D_mint` on the **canonical** chain, (ii) the deposit output was **not** refunded on the canonical chain, and (iii) the epoch's `reg_root_E` backing this mint is the **canonical** registration commitment (not a private-fork / substituted root) and satisfies **epoch-relative** R-09 (`reg_root_E` vs `reg_root_{E-1}` on the unique published lineage — §3.1.2 item 1). The gatekeeper **MUST withhold** the `Pk_mint` signature until all hold. It **MUST NOT** withhold solely because a later epoch head is published. Because the mint cannot settle without the gatekeeper's `Pk_mint` signature (`sk_mint = sk_gk + h`; only the gatekeeper can produce it), a private-fork `MoveToBacked` or a substituted `reg_root_E` yields **no mint**. A co-signed `MoveToBacked` alone is **insufficient** as the sole canonicity gate — its N-of-N setup signature is valid on any fork; the gate is the gatekeeper withholding the **final** mint signature until canonical confirmation of both anchors. The gatekeeper is therefore the **canonical-chain anchor** for **both** `MoveToBacked` **and** `reg_root_E`, not only a source-of-funds / legitimacy check. R-04 closes **Attack A** only; it does **not** close Attack B.
...
**Caveat (MUST state).** Blind signing (signing without checking inputs, including without the canonical-chain check on `MoveToBacked`/`reg_root_E` or the epoch-relative R-09 check) cannot create an unbacked mint that the circuit alone rejects (backing is circuit-enforced against *some* header chain) — but it **does** re-open the private-fork amortization attack of R-04 (Attack A). It does **not** reopen Attack B if R-09 holds in-circuit. Skipping SoF may wave through a tainted mint. The gatekeeper checks all proof inputs, its own canonical Bitcoin view (both anchors), **and** epoch-relative R-09, signs only after those hold, then the proof is built (R-03).
...
| Plonky2 → BitVM2 (SNARK/Groth16) predicate conversion of the zkCoins compliance predicate — integration track (a) | Open engineering (acceptance criteria §4.6B; not demonstrated) | — |
| Open operator-registration market (policy encoding, bond / anti-domination calibration, `reg_root_E`, ceremony) — track (b); semantics + encoding rules §4.1.2 / §4.1.2.1; numbers §4.4 | Spec-side specified; **operational** instantiation still open (G2) | — |
...
BitVM2-mainnet is **CLEARED** (proven). Remaining work is **two gates** on proven components: (a) the Plonky2→BitVM2 predicate-conversion integration gate (§4.6B / G3) **demonstrated**, and (b) the open operator-registration market **operational** (semantics and encoding rules in §4.1.2 / §4.1.2.1; launch numbers in §4.4; onboarding §4.1.3; challenger funding §4.1.4 / §4.6A — G2). Specifying encodings and numbers does **not** clear G2. Both gates, plus §4.6A REQ-4 gates, **MUST** clear. This reframes the document from "future design gated on a not-yet-mainnet verifier" to "**ready-to-implement on proven components once the two remaining gates clear — not a build order until they do**."
...
6. **Anti-domination / anti-Sybil (MUST — R-09 is the Attack-B close; gate A(1) is weaker).** Policy P's bond **MUST** make single-party domination of S economically irrational when `|S| ≥ 2` (cost `ceil(|S|/2) · B_E`; no in-circuit unique-key ≥-half reject — that would ban `|S| = 2`) **and MUST** encode R-09 (growth-only, no kick after admission, every new vault N-of-N over current S). When `|S| = 1` the half-fraction is **vacuous** (genesis self-operator — §4.1.2.1) and **MUST NOT** be treated as a violation. Otherwise a self-controlled **new** vault re-opens **Attack B**. The economic bond alone is **insufficient**. **R-09** is the close. In **gatekeeper mode**, Attack A (private-fork amortization) is blocked by **R-04**; optional R-08 is belt-and-suspenders, not the Attack-B close. **Corner C** (open registration + pooled **without** R-09) is **UNSOUND** (§3.2.1.2 / §5 residual 9 / §6.4).
...
8. **Dead / grieving admitted operators (MUST state).** An admitted operator **cannot be dropped**. A **lost** identity key **permanently** freezes **new** vaults (rotation requires `pk_old`). A **grieving** operator who still holds `pk_old` freezes new vaults until they rotate in place or resume presign. Already-presigned ancestor-epoch vaults remain mintable (R-09 item 1). Existing coins of already-presigned vaults remain exit-able via 1-of-N of that vault's S_E. Safety over mint liveness.

### 4.1.2 Registration commitment, key aggregation, and ceremony robustness

This section specifies the open-registration mechanism that makes the two-level admission of §3.1.2 / §3.3.1 executable. **Semantic requirements (items 1–6) and the canonical Policy-P / `reg_root_E` encoding of §4.1.2.1 are normative now.** Concrete BitVM2-compiler slash/return **script bytes** on the registration output remain **launch parameters** (frozen with the v2 circuit digests, same honesty discipline as §3.1.1 point 4 / NEW-03 connector-byte launch-pin and §4.2.1).

1. **Registration output (bond) format.** Each registrant for epoch E posts a bond of at least the policy-P bond amount to a **slashable registration output** on Bitcoin that commits, in a canonical encoding: `(identity_pubkey, epoch_signing_pubkey pk_{E,i}, epoch_id E, policy_tag = operator_set_root, proof_of_possession)`. The PoP is a signature by `identity_pubkey` over a domain-tagged message binding `(identity_pubkey, E, pk_{E,i})` (NEW-01). Already-admitted identities re-register each epoch only to contribute a **fresh** `pk_{E,i}`; they do not re-join S. The bond output **MUST** be spendable by the epoch's slash/settlement path (so a proven-fraudulent or grief-defaulting operator forfeits it) and returnable to the registrant **only** when every backing-only vault of epoch E has no remaining reimbursement claim path (§4.1.2.1) — a per-claim CSV / "horizon" is **not** enough. **MUST** — no epoch signing key without a live bond output.
...
3. **`reg_root_E` (epoch registration commitment).** `reg_root_E` = a Merkle commitment over S_E, the set of valid registration outputs admitted for epoch E under R-09 (each satisfying items 1–2 within E's registration window, **and** every **identity-party** from `reg_root_{E-1}` — same pubkey, or `pk_new` under a rotation signed by `pk_old`). Epoch signing keys of E−1 are **not** a superset obligation. It **MUST** be published on-chain as part of the epoch anchor (default: dedicated epoch-anchor `OP_RETURN` of the 32-byte `epoch_anchor_id` of §4.1.2.1; alternatively committed in the epoch's `MoveToBacked` graph-anchor leaf if that leaf commits the same 32-byte `epoch_anchor_id`) by the epoch's registering operators / coordinator. The in-circuit statement (via `C_lcp`) **MUST** establish: (i) `reg_root_E` is committed on a **PoW-valid header chain at adequate depth** (same LCP power as `MoveToBacked` — **canonicity is NOT established in-circuit**, exactly as §3.4); (ii) every included registrant satisfies items 1–2 (bond ≥ B, valid PoP, no duplicates); (iii) `agg_key_E` is the key-aggregation of exactly the **epoch signing keys** `{pk_{E,i}}` committed in `reg_root_E`; (iv) **R-09 superset:** every **identity-party** in `reg_root_{E-1}` is represented in `reg_root_E` (same pubkey or signed rotation; genesis epoch E=0 has no predecessor).

   **Canonicity anchor (MUST — R-04 extended; gatekeeper mode).** In gatekeeper mode, the gatekeeper — as part of its **R-04** canonical-view duty — **MUST** confirm on its **own canonical Bitcoin view** that the `reg_root_E` backing this mint's epoch is the **canonical** registration commitment (not a private-fork / substituted root) and that it satisfies **epoch-relative** R-09 (`reg_root_E` vs `reg_root_{E-1}` on the unique published lineage — §3.1.2 item 1). The gatekeeper **MUST** reject a sibling/fork `reg_root`. It **MUST NOT** withhold solely because a later epoch's `reg_root` is already published. R-04's canonical anchor therefore covers **both** `MoveToBacked` **and** `reg_root_E`. In no-gatekeeper mode there is no such Attack-A anchor; Attack B remains closed by R-09.
...
7. **Honest launch-pin.** The concrete registration-output **script template bytes** (the BitVM2-compiler tapleaf programs for slash and return) are **launch parameters** (frozen with the v2 circuit digests, NEW-03 style). The **semantic** requirements (items 1–6) and the **canonical encoding rules** of §4.1.2.1 are normative now. Implementations **MUST NOT** claim present-day absolute byte-determinism for the compiler-emitted slash/return scripts beyond those encoding rules (same honesty as §3.1.1 point 4 / §4.2.1 connector-byte launch-pinning).

#### 4.1.2.1 Policy-P / `reg_root_E` canonical encoding (normative schema)
...
```
leaf_payload :=
    identity_pubkey     ||  // 32-byte x-only
    pk_E_i              ||  // 32-byte x-only epoch signing key
    E                   ||  // u64 big-endian
...
**R-09 superset proof.** For genesis (`E = 0`) there is no predecessor. For `E ≥ 1`, a valid formation **MUST** exhibit a **bijection** from identity-parties in `reg_root_{E-1}` onto a subset of leaves in `reg_root_E`: each predecessor maps to **exactly one** successor leaf, and no successor leaf is the image of two predecessors. A predecessor maps to a leaf whose `identity_pubkey` is that party and `rotation_flag = 0x00`, **or** to a leaf with `rotation_flag = 0x01` and `pk_old` equal to that party's previous identity key. `pk_old` values among rotation leaves **MUST** be unique. A rotation target `identity_pubkey` **MUST NOT** already be a continuing (`rotation_flag = 0x00`) identity from `S_{E-1}`. Cardinality: `|S_E| = |S_{E-1}| + |J_E|` where `J_E` is the set of `rotation_flag = 0x00` leaves whose `identity_pubkey` was **not** in `S_{E-1}`. A `reg_root_E` that omits a predecessor, collapses two predecessors onto one leaf, or rotates onto an occupied identity is **invalid** under P. Epoch signing keys of E−1 are **not** a superset obligation.
...
(all integers `u64` big-endian sats or blocks). `genesis_height` is **immutable** after E=0: every later epoch **MUST** repeat the E=0 value. Advancing `B_E` / windows for a later epoch is announced only in that epoch's other fields; there is no live governance function.

**Epoch start (MUST — deterministic).** `start(0) := genesis_height`. For `E ≥ 1`, `start(E) := start(E-1) + W_epoch_{E-1}` using **that predecessor epoch's** `W_epoch` from its `epoch_params_hash`. Registration for E occupies `[start(E), start(E)+W_reg_E)`. Setup occupies `[start(E)+W_reg_E, start(E)+W_reg_E+W_setup_E)`. Bounds are half-open in blocks: a confirmation at height `h` is in a window iff `start ≤ h < end`.

`epoch_anchor_id := H_tag("zkBTC/v2/EpochAnchor", asset_id ‖ E ‖ prev_anchor_id ‖ prev_root ‖ reg_root_E ‖ epoch_params_hash)`

(`E` is `u64` big-endian; `asset_id` is the 32-byte field of `IssuanceTerms_v3`; `prev_anchor_id` is 32×`0x00` for E=0, else the unique predecessor `epoch_anchor_id`; `prev_root` is 32×`0x00` for E=0 else `reg_root_{E-1}`).

**N-of-N attestation (MUST — not a bare OP_RETURN).** A valid epoch-anchor transaction **MUST** (i) place `0x6a` `0x20` ‖ `epoch_anchor_id` in its first `OP_RETURN` output **and** (ii) carry a MuSig2 (BIP-327) aggregate signature under `agg_key_E` over `H_tag("zkBTC/v2/EpochCert", epoch_anchor_id)` — produced **after** Level-1 completes, over the **final** `reg_root_E` / `agg_key_E`. A bare `OP_RETURN` without that aggregate **MUST** be ignored. Remaining outputs pay the epoch's unslashed-bond return connectors as the compiler template requires. Alternative commitment inside a `MoveToBacked` graph-anchor leaf is permitted only if that leaf commits the same 32-byte `epoch_anchor_id` **and** the same aggregate attestation. E=0 uses the same rule over genesis `S_0 = J_0` after that epoch's Level-1.

**Unique successor (MUST — R-09 item 1).** An attested `epoch_anchor_id` for epoch E is **invalid** if it confirms at height `h < start(E)+W_reg_E+W_setup_E` (before setup ends). `J_E` is provisional until then; `reg_root_E` is published only at setup-window end (§4.1.2 item 6). Among **attested** anchors that confirm at or after that height, the unique successor is the **first** by `(block height, transaction index, output index)` ascending. A later same-`E` attested anchor is a **sibling** and is **invalid**. In gatekeeper mode the gatekeeper's R-04 view **MUST** apply this first-valid-after-setup rule on its canonical chain. In no-gatekeeper mode the LCP proves the same property on the proven header chain (PoW-depth only — canonicity remains the Attack-A residual / `D_mint`). Mechanizing attested first-valid-after-setup is **G3 criterion 5**.

**Registration-output (bond) semantic pin.** The bond UTXO is a P2TR with the vault NUMS internal key of §3.1.1 (no key-path). Its script tree **MUST** contain exactly: (i) a **slash** path spendable only along the epoch's BitVM2 proven-fraud / grief-default graph (bond forfeited; sequencing-connector burn of §4.1 role-note (c); `slash_bounty` paid from this output — §4.1.4); (ii) a **return** path to the registrant's identity-keyed output. The return path **MUST NOT** be spendable until **every** backing-only vault of epoch E has **no remaining reimbursement claim path** (all capacity reimbursed, slashed, or otherwise terminally resolved). A per-claim CSV window is **not** that condition. A reimbursement claim **MUST** prove this operator's bond output is still unspent; after return, further claims of that identity in E are not bounty-backed and **MUST** fail. Exact compiler script bytes for (i) and (ii) remain item-7 launch-pinned. Amount **MUST** be ≥ `B_E`. No `pk_{E,i}` without a live bond output.

#### 4.1.3 Holder → operator onboarding (normative procedure — §4.6A(3))

Open registration is permissionless; it is a **bonded commitment**, not a free click (§4.1 role-note (b)). A wallet that offers "be your own exit agent" / the effectively-trustless product claim **MUST** implement this procedure. Implementations **MUST NOT** construct a deposit for an asset unless the user's operator identity is already in current S (§1.1.1 item 2).

1. **Identity key.** The user generates and **backs up** a BIP-340 identity key. This key is **never** deleted. Loss without a pre-signed rotation freezes **new** vaults of the asset (R-09 item 4 / §4.1.1 item 8). The wallet **MUST** warn that identity-key loss is permanent mint-liveness failure, not a recoverable password.
2. **Join before the first mint.** The wallet **MUST NOT** build a `MoveToBacked` / mint deposit unless `identity_pubkey` is already in the asset's current S **and** was in S_E at presign of every vault that will back a mint. If the asset has already minted and the user was not in those S_E, the wallet **SHOULD** refuse to treat the asset as Attack-B-safe unless at least one operator the user treats as honest already was (§1.1.1 item 2).
3. **Post the bond.** During the registration window of epoch E (§4.4), the user posts ≥ `B_E` sats (product default: 2 BTC class, `B_E = 200000000` sats unless a later epoch raised it) to a slashable registration output (§4.1.2 item 1 / §4.1.2.1). The bond is locked until every backing-only vault of epoch E has no remaining reimbursement claim path (§4.1.2.1); a per-claim CSV window is not enough.
4. **Fresh epoch key + PoP.** The user contributes `pk_{E,i}` with the identity-bound PoP of §4.1.2.1. Already-admitted identities re-register each epoch only for a fresh epoch key; they do not re-join S.
5. **Level-1 then Level-2 ceremony.** The user completes Level-1 operator-set formation, then Level-2 per-deposit graph presign for every admitted deposit of the epoch, inside the setup window (§4.1.2 items 5–6). A not-yet-admitted joiner who misses a per-round deadline is **excluded** from this join and slashed — that joiner was never in S. A new round then begins over `S_{E-1} ∪ (J_E minus the excluded joiner)`, with its own per-round deadline. An already-admitted operator who misses does **not** drop; new vaults stall.
6. **Delete the epoch secret.** After E's presigning completes (Level-1 formation **and** Level-2 presign of all admitted deposits), the user **MUST** irretrievably delete the epoch signing secret for `pk_{E,i}` (NEW-01). The identity key stays.
7. **Serve exit / refuse malice.** Once in S, the user **MAY** front and reclaim their own redemptions from vaults whose S_E includes them, and **MUST NOT** sign a malicious graph for any later vault. That is the Attack-B close and the self-exit path of REQ-4. A late joiner **MUST NOT** be told they can reclaim older vaults they were never in.

**Bonded-onboarding cost (MUST state to the user).** At the product-default pin, joining costs **2 BTC locked** until every vault of that epoch has no remaining reimbursement claim path (§4.1.2.1), plus the time to run Level-1/Level-2 presign, plus Bitcoin fees. This is comparable to Clementine's ~2 BTC operator bond. It is **not** a retail-click onboarding; BitVM3/Glock (§10.1) are the path to cheaper join, not a reason to hide the present cost. A holder who does not pay it still has gatekeeper-independent exit **liveness** via ≥1 registered operator, and **no** Attack-B guarantee of their own.

#### 4.1.4 Challenger incentive / funding (normative — §4.6A(4))

BitVM2 permissionless challenge is **delivered** (anyone **MAY** challenge — §4.1 role-note (a)). Availability ≠ action. Safety still needs ≥1 honest live challenger actually acting in-window (§5 residual 3). Funding is the remaining economic gate.

**Anti-pattern (MUST name).** Citrea Clementine's **deployed** bridge does **not** reimburse challengers: the whitepaper's cross-chain reimbursement of challenge cost is omitted in the live deployment, so the existential-honesty assumption narrows to challengers who self-fund the upfront dispute (Clementine docs, "Challenge Reimbursement" implementation note). zkBTC **MUST NOT** ship that gap. Self-funded-only challenge is the anti-pattern this gate exists to close.

**On-Bitcoin slash bounty (MUST).** A successful disprove / slash of an operator **MUST** pay the acting challenger a `slash_bounty` **in the slash transaction itself**, from the slashed bond, on Bitcoin — not via a cross-chain IOU.

- `slash_bounty := min(bond, max(DISPUTE_COST_CEILING, bond / 10))` with `DISPUTE_COST_CEILING = 16000000` sats (0.16 BTC). Rationale: BitVM2 worst-case on-chain dispute is the ~$16k class (§4.0 / §4.4); at the product-default 2 BTC bond this yields `max(0.16 BTC, 0.20 BTC) = 0.20 BTC` to the challenger, which covers that class with margin. Integer division is floor division on sats.
- The remainder of the slashed bond **MUST** be paid to an unspendable NUMS output (the vault NUMS of §3.1.1 or `OP_RETURN` of value). It **MUST NOT** be paid to other operators, to a club, or to a security council (there is none). A cartel that never challenges then splits slash residual is the failure mode this burn closes.
...
7. **TS3 mint.**
   - **Gatekeeper-gated (MUST — R-04 canonical anchor; R-09 already in-circuit):** the gatekeeper verifies the mint's witness/inputs (source-of-funds, LCP-proven `MoveToBacked` to the asset's instantiated backing-only vault, committed recipient, `operator_set_root` membership and **epoch-relative R-09** (`reg_root_E` vs `reg_root_{E-1}` — §3.1.2 item 1 / §3.2.1.1) against the asset-bound policy root) **and**, on its **own canonical Bitcoin view**, that (i) `MoveToBacked` is confirmed to depth ≥ `D_mint` on the **canonical** chain, (ii) the deposit output was **not** refunded on the canonical chain, and (iii) `reg_root_E` is the canonical registration commitment for the epoch (R-04 covers both anchors — §4.1.2); **only then** does it sign `m_state` under `Pk_mint` (approval = nullifier admission; **withhold until canonical**). It **MUST NOT** withhold solely because a later epoch head is published. The final proof `C` is constructed afterward embedding that signature. Clauses (a)–(h) are proven against the **vault outpoint** (N-of-N witness + instance byte-equality + recursive `operator_set_root` equality + R-09 + `MoveToBacked` depth ≥ `D_mint`). The one-shot mint nullifier `(Pk_mint, R)` is published. Co-signed `MoveToBacked` alone does **not** settle a mint — the gate is the withheld final mint signature (R-04 closes Attack A; R-09 closes Attack B — §3.1.2).
...
and a redeem transition proof such that:
  ...
  8. The Bitcoin payout output is P2TR(btc_recipient) with amount
     ...
     enforce that most-work comparison is an OPEN conversion item
     (G3 — §11); this document does NOT assert it as already
 ...
 12. Non-transferable, claimant-committed, SOLE-FUNDED payout
     ...
     check and any residual economic-attribution question are a G3
     conversion focus. The invariant
...
| Operator bond **class** (identity-bound in P) | **200000000 sats (2 BTC)** (`PROVISIONAL` class pin) | Clementine ~2 BTC (BitVM2-era) | Identity-bound class inside `operator_set_root` (§4.1.2.1). Must match BitVM2 slash economics; slash burns sequencing connector (§4.1 role-note (c)) and pays `slash_bounty` (§4.1.4). BitVM3 later lowers required bonds (§10.1) |
| Per-epoch bond amount `B_E` | **200000000 sats** at genesis; **MAY** raise, **MUST NOT** fall below class (`PROVISIONAL`) | same | Scheduling / economics value; does **not** change `asset_id`. Covers ~$16k-class dispute plus §4.1.4 bounty at the 2 BTC pin |
...
| Minimum independent watchtower count | **≥ 3** (`PROVISIONAL` floor) | PA replaced by sequencing connectors (Clementine/Strata class one-shot) | Floor bounds **independent operators** (each consumes ≤1 one-shot before ejection), not sequential claims of one operator (§4.1 role-note (c)). **Not a launch gate** — residual §5 / §11 item 5 |
| `slash_bounty` / `DISPUTE_COST_CEILING` | `min(bond, max(16000000 sats, bond/10))` (`PROVISIONAL`) | BitVM2 ~$16k dispute class; Clementine self-funded challenge is the **anti-pattern** | Paid to the acting challenger in the slash tx; remainder burned to NUMS (§4.1.4) |
| Registration window `W_reg` | **144 blocks** (`PROVISIONAL`) | Open-operator market §4.1.1 / §4.1.2 | ~24 h for permissionless bond+PoP join before Level-1. Does **not** change `asset_id` |
| Per-round presign deadline `W_round` | **18 blocks** (`PROVISIONAL`) | §4.1.2 item 5 | ~3 h; **not** the setup window. A not-yet-admitted joiner who misses is excluded+slashed; an admitted operator who misses does not drop |
| Setup window `W_setup` | **144 blocks** (`PROVISIONAL`) | BitVM2 N-of-N re-presign cost | ~24 h total for bounded restart rounds (⌊144/18⌋ = 8 rounds). Caps griefing re-run cost |
| Epoch cadence `W_epoch` | **1008 blocks** (`PROVISIONAL`) | shorter than `D_mint` so epochs overlap | ~7 days between registration windows. Ancestor-epoch mints of already-presigned vaults remain valid (R-09 item 1). Does **not** change `asset_id` |

Mark every value **`PROVISIONAL` until BitVM2 launch-pin against live economics.** The numbers above are the **specified intended launch values**, not an assertion that a live market exists. G2 stays open until that market is operational.
...
- verification that **epoch signing secrets** are deleted, that **identity keys remain**, and that no live `agg_key` CHECKSIG leaf remains on vault outputs;
...
Without these, the REQ-4 claim is **false** (a mint-time party re-enters on the safety or liveness axis of exit). Spec-side statements below are normative now. They do **not** clear G2 (operational market) or G3 (conversion demonstrated).

1. **Policy-P anti-domination and R-09 (open registration market).** Policy P **MUST** encode R-09 (growth-only, no kick after admission, every new vault N-of-N over current S) **and** the economic anti-domination target of §4.1.2.1 (`anti_dom_numer/denom = 1/2`, `bond_class_sats = 200000000`): dominating ≥ half of S costs `ceil(|S|/2) · B_E` under honest-registration. This is **not** an in-circuit unique-key headcount (that would reject every `|S| = 2` set). Vacuous at `|S| = 1` (genesis self-operator). **Attack B is closed by R-09**, not by a gatekeeper vouch. (R-04 remains the private-fork / Attack A backstop when a gatekeeper is designated and is independent of this domination gate.) Bootstrap capture of **genesis** S (one organisation == entire S at first mint) reintroduces the prefix residual of R-09 item 6 — hence the first-mint rule.
2. **Setup integrity (BitVM2 form).** For every dispute path, N-of-N presign ceremony integrity **MUST** hold, with the independent-watchtower/challenger floor met and the anti-grief ceremony rule of §4.1.2 item 5. Toxic-waste concern re-based to BitVM2: connector/presign correctness; one honest signer deletes its **epoch signing secret** (NEW-01; identity keys remain). The ceremony **MUST** ensure that no single mint-time party monopolises setup artefacts that would allow forging or voiding fraud proofs.
   **Circuit-specific setup (resolved for the specified conversion path).** The G3 conversion is specified as Plonky2 → **Groth16/SNARK** for BitVM2 verification. Groth16 has a **circuit-specific structured reference string** (toxic waste). Therefore a global circuit-specific setup artifact **does** exist on this path, and the classic **MPC ceremony with independent contributors** gate **MUST** apply to that SRS before any public zkBTC claim. A conversion that instead demonstrates a **transparent** SNARK BitVM2 can still verify **MAY** drop this MPC requirement; until that demonstration, assume Groth16 and keep the MPC gate. This resolution is **not** G3 clearance.
3. **Documented holder → operator onboarding** (self-fronting). The procedure in **§4.1.3** is the onboarding write-up: open permissionless registration (§4.1.1 / §4.1.2 / §4.1.2.1), first-mint rule, and the bonded-onboarding cost of §4.1 role-note (b) (product default: 2 BTC locked per epoch class). It does **not** instantiate the reference operator market (G2 remains operational).
4. **Challenger economics documented.** Naming the Clementine self-funded-challenge anti-pattern is **not** this gate clearing. zkBTC's specified funding path is the on-Bitcoin `slash_bounty` of **§4.1.4**. Permissionless challenge availability is already delivered by BitVM2; this gate is funding, not availability. Emitting the bounty in the live graph is part of G2/G3 instantiation. This gate **MUST NOT** make the gatekeeper a challenger-funder: the gatekeeper has no peg-out role (REQ-4).

#### (B) Maturity / integration gates

Restatement of §4.0 as a checklist (two remaining gates — both **MUST** clear).

- [x] BitVM2 mainnet live (CLEARED — Bitlayer 2025-07; Citrea Clementine live 2026-01)
- [ ] Reference open-registration operator market **operational** (§4.1.1 / §4.1.2 / §4.1.2.1) — track (b). Spec-side encoding, windows, bond class, onboarding, and bounty are specified; **operational** means a live reference market that admits permissionless join under those rules. Specifying the rules does not check this box.
- [ ] **Plonky2 → BitVM2 (Groth16/SNARK) conversion of the zkCoins compliance predicate demonstrated** — track (a)

**G3 acceptance criteria (MUST — all of them, or the gate stays open):**

1. **Headline integration.** A public conversion artifact that proves the zkCoins compliance predicate `C` (parent specification.md) in a Groth16/SNARK form a BitVM2 assert/disprove path can verify, with test vectors, against the Clementine and/or Bitlayer reference verifier shape.
...
5. **First-valid epoch successor.** The converted graph / LCP **MUST** enforce the attested first-valid-after-setup `epoch_anchor_id` rule of §4.1.2.1 (N-of-N `agg_key_E` certificate over the final root; unique R-09 lineage; bound `epoch_params_hash` and `prev_anchor_id`; deterministic `start(E)`).
...
6. **Permissionless challenging is DELIVERED under BitVM2** (not an open dependency — residual 3 / §4.1 role-note (a)); mechanism-availability alone does **not** guarantee safety. The residual beyond delivery is that **≥1 honest challenger actually acts in-window** (residual 3). Challenger **incentive/funding** that makes such action rational is specified as the on-Bitcoin `slash_bounty` of §4.1.4 / §4.6A(4); emitting that bounty in the live graph is part of G2/G3 instantiation.
...
10. **Open-operator anti-domination residual:** policy P **MUST** make single-party domination of S economically irrational when `|S| ≥ 2` (§3.1.2 / §4.6A(1) / §4.1.1 / §4.1.2.1) **and MUST** encode R-09; otherwise a self-controlled **new** vault re-opens **Attack B**. Enforcement is bond cost, not an in-circuit unique-key ≥-half reject (that would ban `|S| = 2`). When `|S| = 1` the half-fraction is vacuous (genesis self-operator). Economic bond/stake enforcement is weaker than R-09 and **insufficient alone**. **Prefix residual:** coins minted before the intended honest operator joined are untrusted because the token is fungible (§3.5.5); the first-mint rule of R-09 item 6 is load-bearing, not a footnote.
11. **Redeem-side payout-canonicity residual (§4.3.2 clause 8) — partly OPEN:** the fraud statement's "payout confirmed on the canonical chain" has **no external anchor** on the redeem side (gatekeeper absent by design — REQ-4), and depth alone does not distinguish a canonical payout from a deep private-fork one. Closing it requires the challenge/disprove to compare **cumulative work** (most-work chain), not mere depth; **whether the concrete BitVM2 disprove enforces that is an OPEN conversion item (G3 — §11 item 14).** Until shown, payout canonicity is an **unmechanized** bound resting on sub-~45-50% Bitcoin hashrate (residual 4) + ≥1 honest active challenger (residual 3). It is the redeem-side analog of the mint reorg residual, but — unlike the mint side, which is closed by the gatekeeper's external R-04 view **when gated** (Corner D leaves Attack A as Bitcoin-class / `D_mint` — residual 9) — **not** closed by a specified mechanism in this document.
...
**Additionally, when designated, three mint-time duties:**

1. **Source-of-funds screening** (reputation / compliance surface). Vault legitimacy is **R-09** in-circuit (§3.1.2), not this duty.
...
3. **Epoch-relative R-09 withhold (MUST when designated; additional, not the Attack-B close).** Withhold `Pk_mint` if `reg_root_E` is a sibling/fork of the unique published lineage. **MUST NOT** withhold an ancestor-epoch mint solely because a later epoch head is published. The Attack-B close remains in-circuit R-09.
...
| Vault legitimacy | In-circuit R-09 / `operator_set_root` (epoch-relative). Optional gated R-08 is belt-and-suspenders, not the close | §3.1.2 / §3.3(e) |
...
1. **Plonky2 → BitVM2 (Groth16/SNARK) conversion** of the zkCoins compliance predicate — **headline blocking integration gate (G3)**; acceptance criteria in §4.6B. Still **not demonstrated**.
2. **Global circuit-specific setup artifact in the conversion path?** **Resolved for the specified Groth16 path:** yes — Groth16 SRS is circuit-specific toxic waste; launch gate A(2) MPC ceremony **MUST** apply unless a transparent SNARK is demonstrated instead (§4.6A(2)). Not G3 clearance.
3. **Exact operator bond and challenge economics under BitVM2.** **Specified intended launch values** in §4.4 / §4.1.2.1 / §4.1.4 (2 BTC class, `B_E = 200000000` sats, `slash_bounty` formula against the ~$16k dispute class). Still tagged `PROVISIONAL` until a live BitVM2 launch-pin confirms them. Specifying the numbers is not G2 operational.
4. **Challenger incentive/funding economics** under permissionless BitVM2 challenge. **Specified:** on-Bitcoin `slash_bounty` of §4.1.4; Clementine self-funded challenge named as the anti-pattern. Graph emission of the bounty is part of G2/G3 instantiation, not a remaining design question.
5. **Watchtower-count floor calibration** (current `PROVISIONAL` ≥ 3) together with sequencing-connector graph structure against one-shot weakness without a Payout Administrator. **Not a launch gate.**
6. **LCP checkpoint governance and `D_mint` / `S` calibration.** Proposal: the v2 circuit release **pins** the checkpoint, `D_mint`, and optional `S` (§3.7.2); advancing any is a circuit-parameter release (new digests), not a live governance function. There is **no** freshness window `W`. **Not a launch gate.**
7. **Denominations final set** after BitVM2 launch-pin economics (including whether/when 0.01 unlocks under §10.1 efficiency upgrades). Start set {0.1, 1, 10} remains `PROVISIONAL`. **Not a launch gate.**
...
11. **Open-registration policy-P encoding.** **Specified** in §4.1.2.1 (tagged-hash policy root, registration leaf, left-complete Merkle tree, R-09 superset proof, default `OP_RETURN` epoch anchor). Compiler slash/return **script bytes** remain item-7 launch-pinned (NEW-03 honesty). Canonicity of `reg_root_E` is a **gatekeeper R-04 duty** when designated; completeness of *new joiners* is a **liveness residual**; completeness of already-admitted keys is **safety** (Attack-B safety = R-09).
12. **Anti-domination bond calibration.** **Specified:** 2 BTC class; economic target 1/2 via `ceil(|S|/2) · B_E`; **not** an in-circuit unique-key ≥-half reject (that would ban `|S| = 2`). Vacuous at `|S| = 1` (self-operator genesis). **R-09** remains the Attack-B close. Live-economics confirmation is the remaining `PROVISIONAL` tag, not a missing rule.
13. **Registration-window / epoch-cadence / setup-window parameters.** **Specified intended launch values** in §4.4: `W_reg = 144`, `W_round = 18`, `W_setup = 144`, `W_epoch = 1008` (blocks). Semantic formation of `reg_root_E` is in §4.1.2; encoding in §4.1.2.1. Still `PROVISIONAL` until live ceremony-cost confirmation. G2 operational remains open.
14. **Redeem-side payout-canonicity enforcement.** The BitVM2 disprove path **MUST** enforce a **most-work (cumulative-work)** comparison so that a deep private-fork "payout" loses to an honest challenger's heavier canonical chain (§4.3.2 clause 8 / §5 residual 11 / §4.6B G3 criterion 3). **Open until G3 demonstrates it**; until then the redeem-side canonicity bound is **unmechanized** (no external anchor exists on the redeem side by design — REQ-4).
...
| 2026-08-20 | Spec-side of G2 / REQ-4: canonical Policy-P and `reg_root_E` encoding (§4.1.2.1; `H_tag` not Poseidon `Hc`; 365-byte leaf including identity `pop_sig` and epoch-key `epoch_pop_sig`; NEW-03 unpaired-hash promotion; `epoch_anchor_id` binds `asset_id`/`E`/`prev_anchor_id`/`prev_root`/`reg_root_E`/`epoch_params_hash` with N-of-N `agg_key_E` attestation and first-valid-after-setup; deterministic `start(E)`; bond return only after epoch vaults have no remaining claim path; Corner-B `genesis_keys_root`; COutPoint `bond_txid` order), intended launch numbers for bond class (2 BTC), `B_E`, windows (`W_reg`/`W_round`/`W_setup`/`W_epoch`), holder→operator onboarding procedure (§4.1.3), and on-Bitcoin `slash_bounty` challenger funding (§4.1.4) with Clementine self-funded challenge named as the anti-pattern. Anti-domination is economic (`ceil(|S|/2) · B_E`), not an in-circuit unique-key ≥-half reject (that would ban `|S| = 2`); vacuous at `|S| = 1`. A(2) resolved for the specified Groth16 path (MPC ceremony required). G3 acceptance criteria listed. **G2 and G3 boxes stay unchecked** — encodings and numbers are not an operational market and not a conversion demonstration. Residuals in §11 items 5–10 and 14 stay residuals, not new launch gates. |
...
| D13 | Open operator registration market: permissionless join into cumulative S (R-09), bond/PoP/`reg_root_E`/MuSig2 KeyAgg/bounded restart rounds (exclusion only of not-yet-admitted joiners; per-round deadline ≠ setup window) + two-level ceremony (§4.1.2), canonical Policy-P / `reg_root_E` encoding (§4.1.2.1), holder→operator onboarding (§4.1.3), `slash_bounty` challenger funding (§4.1.4), be-your-own-exit-agent scoped to EXIT and Attack B, registration-free-reclaim residual → §10.2, first-mint rule §1.1.1 | §4.1.1, §4.1.2, §4.1.2.1, §4.1.3, §4.1.4, §1.1.1, §1.2, §4.3.1 |
...
| 'critical bug → only freeze/burn, not theft' stated unconditionally (§4.5) | Conflated a non-soundness bug's freeze/burn outcome with a circuit/graph soundness bug's theft risk; contradicted §5 'sound BitVM2 crypto' condition | Split by bug class: non-soundness bug = freeze/burn under 1-of-N + sound crypto; circuit/graph **soundness** bug can enable theft (§5 sound-crypto condition); soundness remains a standing assumption |

---

*End of design specification. No code. Not a production launch until the §4.6A REQ-4 gates and §4.6B maturity/integration gates clear (two remaining gates: Plonky2→BitVM2 conversion **demonstrated** and open-registration market **operational**); spec-side encodings and numbers for the latter are in this document and do not themselves clear those gates. The verifier itself is mainnet-proven.*
