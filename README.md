...
- **Operator liveness and cost.** Exit depends on at least one registered operator being live, liquid, and willing to front BTC within the holder's `max_fee`. "Open/permissionless" means anyone *may* serve, not that anyone *will* at a good price. Joining the operator set is a bonded commitment: the specified intended launch class is 2 BTC locked per epoch (Clementine-class), plus ceremony time — not a free click. Thin liquidity can make exit slow or expensive.
...
- **Not production-ready.** No implementation code exists, and two gates (predicate conversion **demonstrated**, open-registration market **operational**) plus the REQ-4 gates must clear first. Spec-side encodings, launch numbers, onboarding, and challenger-funding rules are in the specification; they do not themselves clear those gates.
...
| (a) Compliance-predicate conversion | **Open.** Convert the zkCoins compliance predicate from its current proving system to a form BitVM2 can verify (a Groth16/SNARK conversion) and **demonstrate** it — an integration engineering gate. Acceptance criteria are in the specification §4.6B. |
| (b) Open operator-registration market | **Open.** Instantiate a live **growth-only** operator-registration market under the specification's encoding, bond class, windows, onboarding procedure, and challenger-bounty rules (R-09: identity set, no leave, identity-bound epoch signing keys). Spec-side rules are written; **operational** instantiation is the remaining gate. |

Both remaining gates must clear before this is a build order. BitVM2 itself is mainnet-proven, and the remaining work — predicate conversion and open-registration instantiation — is well-scoped engineering on proven components, not speculative research. That does not make this a build order yet: no code exists here, and it is not production-ready until the gates above clear. Specifying encodings and numbers is not clearance.
